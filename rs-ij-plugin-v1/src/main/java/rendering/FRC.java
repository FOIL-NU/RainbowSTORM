...
public class FRC {
   ...
   // Log log = new Log();
    ...
    public static double[] calculateThresholdCurve(FRCCurve frcCurve, ThresholdMethod method)
    {
        ...
        // See: Heel, M. v. & Schatz, M. Fourier shell correlation threshold criteria. J. Struct. Bio. 151 (2005).
	...
	public Plot doPlot(FRCCurve frc_curve,ThresholdMethod tm, double fire, String name) {
		
		//FRC frc = new FRC();
             // FRCCurveResult[] frc_curve =fCurve.results;
              
             int sz= frc_curve.getSize();
             
             FRCCurve smFRC= getSmoothedCurve(frc_curve, false);
		...
		for(int i=0; i<sz; i++) {
			...
			// we can get a calibrated frequency axis.
                        ...
                        x[i] =frc_curve.get(i).radius;//frc_curve[i]. frc_curve[i].numerator/(2*(frc_curve[i].denominator+1));

			//x[i]  =  frc_curve[i][0]/(2*(frc_curve.length+1));
			
			// Original FRC curve
                       y[i]  =  frc_curve.get(i).correlation;
			//y[i]  =  frc_curve[i][1];
			
			// Smoothed FRC Curve

			sy[i] = smFRC.get(i).correlation;
		}
		...
		double[] thr_curve =  calculateThresholdCurve(frc_curve, tm);
